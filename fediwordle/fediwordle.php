...
/*
	foreach ($answer_arr as $i => $char) {
		...
		if ($char === $word_arr[$i]) {
			...
			$ret['iconfig']['chars'][$i] = $char;
			$res_char_count_arr = array_count_values($ret['iconfig']['chars']);
			if (($res_char_count_arr[$char] == $char_count_arr[$char]) && ($key = array_search($char, $ret['iconfig']['?_chars'])) !== false) {
				unset($ret['iconfig']['?_chars'][$key]);
			}

		}
		elseif (in_array($char, $word_arr) && $process_char_count_arr[$char] > 0) {
			$res[$i] = '🟡';
			$ret['success'] = false;
			$res_char_count_arr = array_count_values($ret['iconfig']['chars']);
			if (!in_array($char, $ret['iconfig']['?_chars']) && $char_count_arr[$char] > $res_char_count_arr[$char]) {
				$ret['iconfig']['?_chars'][$i] = $char;
			}
		}
		else {
			$res[$i]= '🔵';
			$ret['success'] = false;
			if (!in_array($char, $ret['iconfig']['x_chars']) && !in_array($char, $ret['iconfig']['?_chars']) && !in_array($char, $ret['iconfig']['chars'])) {
				$ret['iconfig']['x_chars'][] = $char;
			}
		}
